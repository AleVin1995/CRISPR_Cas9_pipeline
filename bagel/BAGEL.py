...
@click.option('-g', '--gene-column', default='1', help='Column index (0-based) or name for gene symbols (default: 1)')
@click.option('-sg', '--sgrna-column', default='0', help='Column index (0-based) or name for sgRNA identifiers (default: 0)')
...
@click.option('--bf-column-name', default='BF', help='Custom name for the Bayes Factor output column (default: BF)')
def calculate_bayes_factors(
        fold_change, output_file, essential_genes, non_essential_genes, columns_to_test, gene_column, sgrna_column, network_file, align_info,
    ...
    with open(fold_change) as fin:
        ...
        try:
            try:
                ...
                column_labels = [fieldname[x] for x in column_list]
            except ValueError:
                column_labels = columns
                column_list = [x for x in range(len(fieldname)) if
                               fieldname[x] in column_labels]
            print("Using column:  " + ", ".join(column_labels))
        except:
            print("Invalid columns")
            sys.exit(1)

        # Determine gene column index (0-based)
        try:
            gene_col_idx = int(gene_column)
        except ValueError:
            if gene_column in fieldname:
                gene_col_idx = fieldname.index(gene_column)
            else:
                print("Invalid gene column: " + gene_column)
                sys.exit(1)

        # Determine sgRNA column index (0-based)
        try:
            sgrna_col_idx = int(sgrna_column)
        except ValueError:
            if sgrna_column in fieldname:
                sgrna_col_idx = fieldname.index(sgrna_column)
            else:
                print("Invalid sgRNA column: " + sgrna_column)
                sys.exit(1)

        for line in fin:
            fields = line.rstrip().split('\t')
            rnatag = fields[sgrna_col_idx]
            ...
            for i in column_list:
                fc[rnatag][i] = float(fields[i])  # 0-based column index
    ...
    if use_small_sample:
        ...
        training_data = Training(np.where(np.isin(genes_array, np.union1d(coreEss, nonEss)))[0],
                                 cvnum=no_of_cross_validations)
        # all non-goldstandards
        all_non_gs = np.where(np.logical_not(np.isin(genes_array, np.union1d(coreEss, nonEss))))[0]
    else:
    ...
    for loop in range(LOOPCOUNT):
        ...
        train_ess = np.where(np.isin(genes_array[gene_train_idx], coreEss))[0]
        train_non = np.where(np.isin(genes_array[gene_train_idx], nonEss))[0]
