process RUN_MAGECK {
    ...
    publishDir "${params.mageck_dir}", mode: 'copy', pattern: "*.mageck*"

    input:
    tuple val(batch_name), path(input_file)

    output:
    path "*.mageck*", emit: mageck, optional: true

    script:
    """
    # Replace anything after "." or "_"
    cell_line_basename=\${input_file.baseName%%[._]*}

    gunzip -c ${input_file} > /tmp/\${cell_line_basename}.tsv

    design_matrix_file=/tmp/\${cell_line_basename}_design_matrix.tsv

    # Generate the design matrix for MAGeCK
    if Rscript ${projectDir}/../modules/templates/design_matrix_mageck.R \
        /tmp/\${cell_line_basename}.tsv \
        ...
        mageck mle -k /tmp/\${cell_line_basename}.tsv \
